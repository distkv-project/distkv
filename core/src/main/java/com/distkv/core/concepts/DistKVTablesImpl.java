...
public class DistKVTablesImpl extends DistKVConcepts<TableEntry> implements DistKVTables {

  public DistKVTablesImpl() {
    distKVKeyValueMap = new DstHashMapImpl<>();
  }
  ...
  public void createTable(TableSpecification tableSpec) {
    ...
    distKVKeyValueMap.put(tableSpec.getName(), table);
  }
  ...
  public void append(String tableName, List<Record> sourceRecords) {
    ...
    TableEntry tableEntry = distKVKeyValueMap.get(tableName);
  ...
  public TableSpecification getTableSpecification(String tableName) {
    checkTableExists(tableName);
    return distKVKeyValueMap.get(tableName).getTableSpec();
  }

  @Override
  public List<Record> query(String tableName, Map<Field, Value> conditions) {
    checkTableExists(tableName);
    List<Record> records = distKVKeyValueMap.get(tableName).getRecords();
    ...
    for (Map.Entry<Field, Value> entry : conditions.entrySet()) {
      ...
      if (primary || index) {
        Index indexs = distKVKeyValueMap.get(tableName).getIndex();
        ...
        TableSpecification tableSpec = distKVKeyValueMap.get(tableName).getTableSpec();
        List<Field> fields = tableSpec.getFields();
  ...
  public void clearTable(String tableName) {
    TableEntry table = distKVKeyValueMap.get(tableName);
  ...
  private boolean isExist(String tableName) {
    return distKVKeyValueMap.containsKey(tableName);
  }
}
